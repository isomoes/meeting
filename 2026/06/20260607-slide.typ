...
#redt[防护范围受限：三种 CED 方案只覆盖 4-bit S 盒，未保护其余数据通路。]

- 线性扩散层、密钥加、置换等环节注入的故障 #redt[不在检测范围]；
- 与攻击侧呼应：DFA 多在末轮 S 层附近注入，S 盒保护虽抓住关键点，但 #bluet[非全数据通路防护]；
- 应对：明确给出威胁模型与防护边界，或讨论扩展到线性层 / 寄存器的开销。

#centered-slide[
  #text(size: 2em, weight: "bold")[谢谢！]
]
